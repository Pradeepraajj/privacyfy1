...
# --- SIMPLIFIED ENCRYPTION CONFIG ---
# This ensures that even without complex signatures, your demo download works
STATIC_ENCRYPTION_SECRET = os.getenv("ENCRYPTION_SECRET", "privacyfy_demo_secret_2026")
...
async def verify_document(
    ...
    upload_type: str = Form(...) # Recieved from frontend to determine path
):
    ...
    # --- BYPASS AI FOR PERSONAL DOCUMENTS ---
    if upload_type == 'personal':
        ...
        return {
            "valid": True,
            "doc_hash": doc_hash,
            "details": {"document_type": "Personal", "status": "SKIP_AI"},
            "message": "Personal Document staging successful (AI Bypassed)."
        }

    # --- RUN AI FOR GOVERNMENT DOCUMENTS ---
...
async def finalize_upload(
    ...
    try:
        # --- PHASE 2: STATIC-KEY ENCRYPTION (FOR DEMO STABILITY) ---
        # Using a fixed secret key ensures that the lock and key always match
        key = hashlib.sha256(STATIC_ENCRYPTION_SECRET.encode()).digest()
        cipher = AES.new(key, AES.MODE_CBC)
        iv = cipher.iv

        # Encrypt the raw bytes
        raw_data = staged["content"]
        encrypted_data = cipher.encrypt(pad(raw_data, AES.block_size))

        # Combine IV + Encrypted Data into Base64
        final_payload = base64.b64encode(iv + encrypted_data).decode('utf-8')
        
        # Save to temp text file for Pinata
        enc_path = f"enc_{doc_hash}.txt"
        with open(enc_path, "w") as f:
            f.write(final_payload)
        
        final_cid = upload_to_ipfs(enc_path)
